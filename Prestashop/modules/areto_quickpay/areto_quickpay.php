...
class areto_quickpay extends PaymentModule
{
    private $_html = '';
    ...
    public function __construct()
    {
        ...
        // Init Configuration
        $config = Configuration::getMultiple(array('ARETO_QP_API_ID', 'ARETO_QP_API_SESSION', 'ARETO_QP_TERMS_URL', 'ARETO_QP_DESCRIPTOR'));
        $this->api_id = isset($config['ARETO_QP_API_ID']) ? $config['ARETO_QP_API_ID'] : '';
        $this->api_session = isset($config['ARETO_QP_API_SESSION']) ? $config['ARETO_QP_API_SESSION'] : '';
        $this->terms_url = isset($config['ARETO_QP_TERMS_URL']) ? $config['ARETO_QP_TERMS_URL'] : '';
        $this->descriptor = isset($config['ARETO_QP_DESCRIPTOR']) ? $config['ARETO_QP_DESCRIPTOR'] : '';
        ...
        $this->displayName = $this->l('Areto Systems QuickPay');
        $this->description = $this->l('Start accepting online credit card payments through Areto Systems');
...
");
        ...
        Configuration::updateValue('ARETO_QP_API_ID', '');
        Configuration::updateValue('ARETO_QP_API_SESSION', '');
        Configuration::updateValue('ARETO_QP_TERMS_URL', '');
        Configuration::updateValue('ARETO_QP_DESCRIPTOR', '');
        return true;
    }
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            Configuration::updateValue('ARETO_QP_API_ID', Tools::getValue('api_id'));
            Configuration::updateValue('ARETO_QP_API_SESSION', Tools::getValue('api_session'));
            Configuration::updateValue('ARETO_QP_TERMS_URL', Tools::getValue('terms_url'));
            Configuration::updateValue('ARETO_QP_DESCRIPTOR', Tools::getValue('descriptor'));
        }
    ...
    {
        $this->_html .= '<img src="../modules/areto_quickpay/logo.png" style="float:left; margin-right:15px;" width="86" height="49"><b>'
            . $this->l('This module allows you to accept online credit card payments through') . ' <a href="http://www.aretosystems.com">Areto Systems</a></b><br /><br />';
        ...
        $this->_html .=
			...
			<legend><img src="../img/admin/contact.gif" />' . $this->l('Settings') . '</legend>
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr>
					<td colspan="2">' . $this->l('Please specify Areto Systems account details.') . '.<br /><br />
					</td>
					</tr>
					<tr>
					...
					<tr>
						<td width="130" style="vertical-align: top;">' . $this->l('Terms and Conditions URL') . '</td>
						<td><input type="text" name="terms_url" value="' . htmlentities(Tools::getValue('terms_url', $this->terms_url), ENT_COMPAT, 'UTF-8') . '" style="width: 300px;" /></td>
					</tr>
					<tr>
						<td width="130" style="vertical-align: top;">' . $this->l('Descriptor') . '</td>
						<td><input type="text" name="descriptor" value="' . htmlentities(Tools::getValue('descriptor', $this->descriptor), ENT_COMPAT, 'UTF-8') . '" style="width: 300px;" /></td>
					</tr>
    ...
    public function displayError($message)
    {
        ...
        $link = $this->context->link->getModuleLink('areto_quickpay', 'error');
        Tools::redirect($link);
    }
}
